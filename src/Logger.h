...
public:
    Logger()
    {
        ...
        if (!this->ofs)
        {
            this->log(LogType::ERROR_LOG, "Failed to open log file.");
        }
    ...
    void log(LogType type, std::string message)
    {
        ...
        else if (type == LogType::ERROR_LOG)   type_str = "[ERROR]: ";
...
#define ERROR(message)   NodeGui::get().m_logger->log(LogType::ERROR_LOG, message)
